...
//				searchCol.appendChild(TagFactory.createDiv("col-xs-1 align-right", cell.getTitle()));
...
//					TagFactory.createA(uri + mid + ".page", "ace-icon fa fa-search orange bigger-130", "查询"));
//			// 显示隐藏搜索条件按钮
...
//			HtmlTag widgetToolbar = TagFactory.createDiv("widget-toolbar"); // 模块按钮的toolbar
//			int actionCount = 0;
//			for (Action action : actions) {
//				if ("0".equals(action.getType())) {
//					widgetToolbar
//							.appendChild(TagFactory.createA(action.getHref(), action.getClazz(), action.getTitle()));
...
//			for (Cell cell : cells) {
//				if (cell.getDisplay()) {
//					tr.appendChild(TagFactory.createTd("center", StringUtils.toString(map.get(cell.getName()))));
//				}
//			}
//			if (null != actions && actions.size() > 0) {
//				for (Action action : actions) {
//					HtmlTag td = TagFactory.createTd("center visible-md visible-lg hidden-sm hidden-xs action-buttons");
